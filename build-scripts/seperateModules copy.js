...
async function zipDeviceFolder(rootDir, device) {
   return new Promise((resolve, reject) => {
      const deviceDir = path.join(rootDir, device);
      const zipFile = path.join(rootDir, `${device}.zip`);
      exec(`zip -r ${zipFile} ${device}`, { cwd: rootDir }, (error, stdout, stderr) => {
         if (error) {
            console.error(`Error zipping folder for ${device}:`, stderr);
            reject(error);
         } else {
            console.log(`${device}.zip has been created successfully.`);
            resolve();
